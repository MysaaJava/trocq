...
Elpi Accumulate lp:{{
  solve InitialGoal NewGoals :- debug dbg.none => std.do! [
    InitialGoal = goal _Context _ G _ TacticArgs,
    coq.say "Got args" TacticArgs,
    std.assert-ok! (coq.typecheck G Ty) "goal ill-typed",
    coq.say "Juste en dessous !!!!!!!!!!!!!!!!",
    util.when-debug dbg.full (coq.say "goal HERE!" G),
    %std.assert-ok! (TacticArgs = [trm G']) "There should be one argument that is a term",
    TacticArgs = [trm G'],
    coq.say "!!!" G',
    %std.assert-ok! (coq.typecheck G' Ty') "tactic parameter is ill-typed",
    util.when-debug dbg.full (coq.say "objective goal" G'),
...
(* Tactic Notation "trocqoercion" constr(trm) := elpi trocqoercion (trm). *)
